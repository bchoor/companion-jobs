...
export function extractFestoolrecon(html: string): ExtractResult {
  ...
  function extractBlock(child: ChildNode, pos: number): ContentBlock | null {
    ...
    // Use numeric nodeType constant (3 = TEXT_NODE) since linkedom may not expose Node global
    if (child.nodeType === 3) {
  ...
  if (descElement) {
    ...
    for (const child of descElement.childNodes) {
      ...
      // Use numeric nodeType constant (1 = ELEMENT_NODE)
  ...
  function isDuplicateOfFeature(text: string): boolean {
    ...
    return featureTexts.some((ft) => n.includes(ft) || ft.includes(n));
  }
  ...
  if (whatsIncluded.length === 0 && productSection) {
    const allText = productSection.textContent || "";
    const match = allText.match(/Includes[:\s]+([\s\S]*?)(?:Part number|$)/i);
    if (match) {
